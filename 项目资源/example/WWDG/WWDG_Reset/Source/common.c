...
/*------------------------------------------variables-------------------------------------------*/
uint32_t g_wwdg_window  = 0x20;
uint32_t g_wwdg_counter = 0x0A;
...
{
    /* WWDG时钟使能 */
    std_rcc_apb1_clk_enable(RCC_PERIPH_CLK_WWDG);

    /* WWDG参数配置 */
    std_wwdg_set_prescaler(WWDG_PRESCALER_128);
    std_wwdg_set_window(g_wwdg_window);
...
void led_init(void)
{
    ...
    /* 初始化GPIO */
    std_gpio_init(LED1_GPIO_PORT, &led_io);
