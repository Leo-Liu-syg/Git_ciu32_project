...
void adc_init(void)
{
    ...
    /* 使能通道TS */
    std_adc_internal_channel_enable(ADC_INTERNAL_CHANNEL_VTSEN);
    ...
    */
    std_delayus(ADC_VTS_CHANNEL_DELAY);

    /* 选择通道VTS */
    std_adc_fix_sequence_channel_enable(ADC_CHANNEL_VTS);
    
    /* 配置通道TS采样时间选择组2 */
    std_adc_channel_sample_time_select(ADC_SAMPTIME_GROUP_2, ADC_CHANNEL_VTS);
