...
/*--------------------------------------------define--------------------------------------------*/
/* LCD驱动电压稳定时间。根据不同的VLCD输出电压和电荷泵时钟分频值，调整该值。具体应用可参考CIU32L051的数据手册(tSTAB参数) */
#define LCD_DRIVER_VOLTAGE_DELAY                      (100U)
...
void gpio_init(void)
{
    ...
    */
    ...
    /* 端口 A */    
    lcd_gpio_config.pin = GPIO_PIN_4 | GPIO_PIN_5 | GPIO_PIN_6 | GPIO_PIN_7\
...
{ 
    ...
    lcd_config.bias = LCD_BIAS_1_4;
    lcd_config.duty = LCD_DUTY_1_8;
    lcd_config.driver_mode = LCD_DRIVER_MODE_CHARGE_PUMP;
    lcd_config.prescaler = LCD_PRESCALER_4;                  /* 配置帧速率为64Hz，根据应用需求(平衡显示效果和功耗)，调整此参数 */
    lcd_config.divider = LCD_DIVIDER_16;
    lcd_config.vlcd_voltage = LCD_CHAGE_PUMP_1_4_BIAS_5_00V; /* VLCD电压为5V */
    
    /* LCD 初始化 */
    std_lcd_init(&lcd_config);

    /* 等待输出驱动电压稳定 
       当前为电荷泵时钟16分频下的输出驱动电压稳定时间 */
