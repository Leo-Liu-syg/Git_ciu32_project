...
void crc_init(void)
{
    std_crc_init_t crc_init_param;
    
    /* 使能CRC时钟 */
    std_rcc_ahb_clk_enable(RCC_PERIPH_CLK_CRC);

    /* CRC初始化 */
    crc_init_param.set_iv_flag = CRC_USE_INIT_VALUE;    
    crc_init_param.init_value = CRC_DEFAULT_INIT_VALUE;
    crc_init_param.poly_sel = CRC_POLY_32;
    std_crc_init(&crc_init_param);
}
